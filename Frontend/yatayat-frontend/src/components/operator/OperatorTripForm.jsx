export default function OperatorTripForm({ form, eligibility, submitting, submitLabel, onChange, onSubmit, onCancel }) {
  return (
    <form onSubmit={onSubmit} className="space-y-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <label className="block text-sm font-black text-slate-700">Boarding notes <span className="font-semibold text-slate-400">(optional)</span>
        <textarea maxLength={1000} rows={4} value={form.boardingNotes} onChange={(event) => onChange("boardingNotes", event.target.value)} className="mt-2 w-full rounded-xl border border-slate-300 px-4 py-3 font-medium outline-none focus:border-[#08264a]" placeholder="Passenger boarding instructions" />
...
function Select({ label, value, onChange, children, required }) {
  return <label className="block text-sm font-black text-slate-700">{label}<select required={required} value={value} onChange={(event) => onChange(event.target.value)} className="mt-2 w-full rounded-xl border border-slate-300 bg-white px-4 py-3 font-medium outline-none focus:border-[#08264a]">{children}</select></label>;
}

function Field({ label, value, onChange, ...props }) {
  return <label className="block text-sm font-black text-slate-700">{label}<input {...props} value={value} onChange={(event) => onChange(event.target.value)} className="mt-2 w-full rounded-xl border border-slate-300 px-4 py-3 font-medium outline-none focus:border-[#08264a]" /></label>;
}
