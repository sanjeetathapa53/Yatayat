    ...
    <OperatorLayout>
      <div className="space-y-5">
        <header className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            <h1 className="text-2xl font-semibold sm:text-3xl">Driver Management</h1>
            <p className="mt-1 text-sm text-slate-500">Manage active drivers and invite approved drivers who are currently available.</p>
          </div>
          <button type="button" onClick={refresh} disabled={loading} className="tap-target flex items-center justify-center gap-2 rounded-xl border border-slate-300 bg-white px-5 py-2.5 text-sm font-semibold disabled:opacity-50">
        ...
        {loading ? (
          <div className="flex min-h-48 items-center justify-center"><Loader2 size={32} className="animate-spin" /></div>
        ) : activeTab === "active" ? (
...
function TabButton({ active, onClick, icon, label, count }) {
  return (
    <button type="button" onClick={onClick} className={`flex flex-1 items-center justify-center gap-2 rounded-xl px-3 py-2.5 text-sm font-semibold transition ${active ? "bg-[#08264a] text-white" : "text-slate-500 hover:bg-slate-50"}`}>
          ...
          {drivers.map((driver) => (
            <article key={driver.associationId} className="rounded-2xl border border-slate-200 p-5">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0"><h2 className="safe-wrap text-lg font-semibold text-slate-900">{driver.driverName}</h2><p className="mt-1 flex items-center gap-2 text-sm text-slate-500"><Mail size={14} /> <span className="safe-wrap">{driver.driverEmail}</span></p><p className="mt-1 flex items-center gap-2 text-sm text-slate-500"><Phone size={14} /> {driver.driverPhone || "Phone unavailable"}</p></div>
                <span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-semibold text-emerald-700">{driver.associationStatus}</span>
              </div>
              <div className="mt-5 rounded-xl bg-slate-50 p-4">
                <p className="flex items-center gap-2 text-xs font-semibold uppercase text-slate-500"><Bus size={15} /> Assigned buses</p>
              ...
              <button type="button" disabled={removing === driver.associationId} onClick={() => onRemove(driver)} className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl border border-red-200 bg-red-50 py-2.5 text-sm font-semibold text-red-700 hover:bg-red-100 disabled:opacity-50">
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
      <form onSubmit={onSearch} className="flex flex-col gap-3 sm:flex-row">
        <label className="flex flex-1 items-center gap-3 rounded-xl border border-slate-300 bg-slate-50 px-4"><Search size={18} className="text-slate-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search by name, email or phone" className="w-full bg-transparent py-2.5 outline-none" /></label>
        <button type="submit" disabled={searching} className="rounded-xl bg-[#08264a] px-6 py-2.5 font-semibold text-white disabled:opacity-50">{searching ? <Loader2 size={18} className="mx-auto animate-spin" /> : "Search"}</button>
      </form>
        ...
        <div className="mt-6 grid gap-4 xl:grid-cols-2">
          {drivers.map((driver) => (
            <article key={driver.driverId} className="flex flex-col justify-between gap-4 rounded-2xl border border-slate-200 p-5 sm:flex-row sm:items-center">
              <div className="min-w-0"><h2 className="safe-wrap font-semibold text-slate-900">{driver.fullName}</h2><p className="safe-wrap mt-1 text-sm text-slate-500">{driver.email}</p><p className="mt-1 text-sm text-slate-500">{driver.phone || "Phone unavailable"}</p><span className="mt-3 inline-flex rounded-full bg-emerald-100 px-3 py-1 text-xs font-semibold text-emerald-700">APPROVED</span></div>
              <button type="button" disabled={inviting === driver.driverId} onClick={() => onInvite(driver)} className="flex shrink-0 items-center justify-center gap-2 rounded-xl bg-blue-50 px-4 py-2.5 text-sm font-semibold text-[#08264a] hover:bg-blue-100 disabled:opacity-50">{inviting === driver.driverId ? <Loader2 size={16} className="animate-spin" /> : <UserPlus size={16} />} Invite Driver</button>
            </article>
          ))}
        </div>
      )}
...
function EmptyState({ icon, title, text }) {
  return <div className="flex min-h-56 flex-col items-center justify-center rounded-2xl bg-slate-50 p-6 text-center text-slate-500"><span className="text-slate-400">{icon}</span><h2 className="mt-4 text-lg font-semibold text-slate-800">{title}</h2><p className="mt-2 max-w-md text-sm font-semibold">{text}</p></div>;
}
