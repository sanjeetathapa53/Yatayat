...
export default function CreateOperatorTripPage() {
  ...
  return <OperatorLayout><div className="mx-auto max-w-5xl space-y-5"><div><h1 className="text-2xl font-semibold">Create Scheduled Trip</h1><p className="mt-1 text-sm text-slate-500">Assign an eligible route, bus, and driver.</p></div>{error && <div className="rounded-2xl border border-red-200 bg-red-50 p-4 font-bold text-red-700">{error}</div>}{loading ? <div className="flex min-h-48 items-center justify-center"><Loader2 className="animate-spin" size={32} /></div> : eligibility && <><EligibilityNotice eligibility={eligibility} /><OperatorTripForm form={form} eligibility={eligibility} submitting={submitting} submitLabel="Create Trip" onChange={(key, value) => setForm((current) => ({ ...current, [key]: value }))} onSubmit={submit} onCancel={() => navigate("/operator/trips")} /></>}</div></OperatorLayout>;
}
