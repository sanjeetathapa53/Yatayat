    ...
    <OperatorLayout>
      <div className="space-y-6">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            <h1 className="text-3xl font-black">Local Services</h1>
            <p className="mt-1 text-sm text-slate-500">Assign approved buses and active associated drivers to local routes.</p>
          </div>
          <button type="button" onClick={openCreate} className="flex items-center justify-center gap-2 rounded-xl bg-[#08264a] px-5 py-3 font-black text-white">
        ...
        <div className="grid gap-3 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm md:grid-cols-[1fr_auto] md:items-end">
          <label className="text-xs font-black uppercase text-slate-500">
            Service date
            <input type="date" value={filterDate} onChange={(event) => setFilterDate(event.target.value)} className="mt-2 w-full rounded-xl border border-slate-300 px-3 py-3 font-bold normal-case outline-none focus:border-[#08264a]" />
          </label>
          <button type="button" onClick={() => { setFilterDate(""); setFilterStatus("ALL"); }} className="rounded-xl border border-slate-300 px-5 py-3 text-sm font-black text-slate-700">
            Clear Filters
          </button>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {["ALL", ...LOCAL_SERVICE_STATUSES].map((status) => (
            <button key={status} type="button" onClick={() => setFilterStatus(status)} className={`rounded-full px-4 py-2 text-xs font-black ${filterStatus === status ? "bg-[#08264a] text-white" : "border border-slate-200 bg-white text-slate-600"}`}>
        ...
        <button type="button" onClick={load} disabled={loading} className="flex items-center gap-2 text-sm font-black disabled:opacity-50">
...
function EmptyLocalServices({ onCreate }) {
  return (
    <div className="rounded-3xl border border-slate-200 bg-white px-6 py-16 text-center shadow-sm">
      ...
      <h2 className="mt-5 text-xl font-black">No local service assignments found</h2>
      ...
      <button type="button" onClick={onCreate} className="mt-5 rounded-xl bg-[#08264a] px-5 py-3 font-black text-white">Create Assignment</button>
    </div>
  );
}
  ...
  return (
    <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-start">
        ...
        <div className="flex flex-wrap gap-2">
          {editable && <button type="button" onClick={() => onEdit(run)} className="flex items-center gap-2 rounded-xl border border-slate-300 px-4 py-2 text-sm font-black text-slate-700"><Edit3 size={16} /> Edit</button>}
          {editable && <button type="button" onClick={() => onCancel(run)} className="rounded-xl border border-red-200 bg-red-50 px-4 py-2 text-sm font-black text-red-700">Cancel</button>}
        </div>
      </div>
      ...
      {run.notes && <p className="safe-wrap mt-4 rounded-2xl bg-slate-50 p-4 text-sm font-semibold text-slate-600">{run.notes}</p>}
    </article>
  );
}
...
function LocalServiceForm({ form, options, editing, saving, onChange, onClose, onSubmit }) {
  return (
    <div className="responsive-modal-backdrop fixed inset-0 z-50 flex justify-center bg-black/50 sm:items-center">
      <form onSubmit={onSubmit} className="responsive-modal-panel w-full max-w-4xl rounded-3xl bg-white p-5 shadow-2xl sm:p-6">
        <div className="mb-6 flex items-start justify-between gap-4">
          <div>
            <h2 className="text-2xl font-black">{editing ? "Edit Local Service" : "Create Local Service"}</h2>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <label className="md:col-span-2 text-xs font-black uppercase text-slate-500">
            Notes
            <textarea value={form.notes} onChange={(event) => onChange("notes", event.target.value)} rows={3} className="mt-2 w-full rounded-xl border border-slate-300 px-3 py-3 font-semibold normal-case outline-none focus:border-[#08264a]" placeholder="Optional dispatch notes" />
          </label>
        </div>
        <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
          <button type="button" onClick={onClose} className="rounded-xl border border-slate-300 px-5 py-3 font-black text-slate-700">Close</button>
          <button type="submit" disabled={saving} className="rounded-xl bg-[#08264a] px-5 py-3 font-black text-white disabled:opacity-60">{saving ? "Saving..." : editing ? "Save Changes" : "Create Assignment"}</button>
        </div>
      </form>
    </div>
  );
...
function Field({ label, type, value, onChange, required }) {
  return <label className="text-xs font-black uppercase text-slate-500">{label}<input type={type} value={value} onChange={(event) => onChange(event.target.value)} required={required} className="mt-2 w-full rounded-xl border border-slate-300 px-3 py-3 font-bold normal-case outline-none focus:border-[#08264a]" /></label>;
}

function Select({ label, value, onChange, required, children }) {
  return <label className="text-xs font-black uppercase text-slate-500">{label}<select value={value} onChange={(event) => onChange(event.target.value)} required={required} className="mt-2 w-full rounded-xl border border-slate-300 px-3 py-3 font-bold normal-case outline-none focus:border-[#08264a]">{children}</select></label>;
}
