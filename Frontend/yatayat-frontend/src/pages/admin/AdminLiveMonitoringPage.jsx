      ...
      <div className="space-y-5">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex flex-col gap-3 lg:flex-row">
            <label className="flex flex-1 items-center gap-3 rounded-xl border border-slate-300 bg-slate-50 px-4 py-2.5">
            ...
            <button type="button" onClick={() => load(true)} disabled={refreshing}
              className="flex items-center justify-center gap-2 rounded-xl border border-[#08264a] px-4 py-2.5 text-sm font-semibold hover:bg-blue-50 disabled:opacity-60">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {FILTERS.map((item) => (
              <button key={item} type="button" onClick={() => setFilter(item)} aria-pressed={filter === item}
                className={`rounded-full border px-3 py-1.5 text-xs font-semibold ${filter === item ? "border-[#08264a] bg-[#08264a] text-white" : "border-slate-200 text-slate-600 hover:border-slate-400"}`}>
                {labelOf(item)}
              </button>
            ))}
        ...
        {loading && <div className="h-[640px] animate-pulse rounded-2xl bg-slate-100" aria-label="Loading live monitoring" />}
        ...
        {!loading && vehicles.length > 0 && (
          <section className="grid min-h-[640px] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm xl:grid-cols-[370px_1fr]">
            <aside className="max-h-[640px] overflow-y-auto border-b border-slate-200 p-4 xl:border-b-0 xl:border-r">
              <div className="flex justify-between"><h2 className="font-semibold">Vehicles</h2><span className="text-xs font-bold text-slate-500">{filtered.length} shown</span></div>
                ...
                {filtered.map((vehicle) => (
                  <button key={keyOf(vehicle)} type="button" onClick={() => select(vehicle)}
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div><p className="font-semibold">{vehicle.busNumber}</p><p className="text-xs font-bold text-slate-500">{vehicle.routeName}</p></div>
            ...
            <div className="grid min-h-[640px] lg:grid-rows-[1fr_auto]">
              <div className="relative min-h-[450px]">
                ...
                {filtered.length > 0 && filtered.every((item) => !hasCoordinates(item)) && (
                  <div className="absolute left-1/2 top-5 z-[500] -translate-x-1/2 rounded-xl bg-white p-4 text-center shadow-xl"><MapPinOff className="mx-auto" /><p className="mt-2 font-semibold">No usable GPS coordinates</p></div>
                )}
              </div>
  ...
  return (
    <div className="border-t border-slate-200 p-5">
      <div className="flex flex-wrap justify-between gap-3">
        <div><h2 className="text-xl font-semibold">{vehicle.busNumber} · {vehicle.busName}</h2><p className="text-sm font-semibold text-slate-500">{vehicle.operatorName}</p></div>
...
function Summary({ label, value, icon, tone = "blue" }) {
  ...
  return <div className="rounded-2xl border bg-white p-4 shadow-sm"><div className={`flex h-9 w-9 items-center justify-center rounded-xl ${colors[tone]}`}>{icon}</div><p className="mt-3 text-[11px] font-semibold uppercase text-slate-500">{label}</p><p className="text-2xl font-semibold">{value}</p></div>;
}
function Badge({ value }) {
  ...
  return <span className={`rounded-full px-2.5 py-1 text-[10px] font-semibold ${styles[value] || styles.OFFLINE}`}>{symbol} {labelOf(value)}</span>;
}
function Detail({ label, value }) { return <div><dt className="text-xs font-semibold uppercase text-slate-400">{label}</dt><dd className="mt-1 font-bold text-slate-700">{value || "Unavailable"}</dd></div>; }
function Link({ label, onClick }) { return <button type="button" onClick={onClick} className="rounded-lg border px-3 py-2 text-xs font-semibold hover:bg-blue-50">{label}</button>; }
function Notice({ title, children, tone = "amber" }) { return <div className={`rounded-2xl border p-4 text-sm ${tone === "red" ? "border-red-200 bg-red-50 text-red-800" : "border-amber-200 bg-amber-50 text-amber-800"}`}><p className="font-semibold">{title}</p><p className="mt-1 font-semibold">{children}</p></div>; }
