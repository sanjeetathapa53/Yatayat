    ...
    <AdminLayout title={title} subtitle={subtitle}>
      <div className="space-y-5">
        <section className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
          <Link to={`/admin/dashboard?range=${range}`} className="flex items-center gap-2 text-sm font-semibold text-[#08264a] hover:underline focus-visible:outline-2">
            <ArrowLeft size={17} /> Back to Dashboard
          </Link>
          <div className="flex gap-2">
            {[["LAST_7_DAYS", "7 Days"], ["LAST_30_DAYS", "30 Days"]].map(([value, label]) => (
              <button key={value} type="button" onClick={() => updateRange(value)} className={`rounded-xl px-4 py-2 text-sm font-semibold ${range === value ? "bg-[#08264a] text-white" : "bg-slate-100 text-slate-700 hover:bg-slate-200"}`}>{label}</button>
            ))}
...
function UsersDetails({ data }) {
  return <><CardGrid items={[["Total Users", data.totalUsers], ["Passengers", data.passengers], ["Drivers", data.drivers], ["Operators", data.operators], ["Admins", data.admins], ["Today", data.registrationsToday], ["In Range", data.registrationsInRange]]} /><div className="grid gap-4 xl:grid-cols-2"><Trend title="Daily registrations" data={data.dailyRegistrations} dataKey="count" /><Distribution title="Role distribution" values={data.roleDistribution} /><Distribution title="Authentication providers" values={data.providerDistribution} /><Recent title="Recent registrations" rows={data.recentRegistrations} columns={[["Role", "role"], ["Provider", "provider"], ["Registered", "createdAt"]]} /></div></>;
}

function OperationsDetails({ data }) {
  return <><CardGrid items={[["Total Buses", data.totalBuses], ["Active Buses", data.activeBuses], ["Pending Buses", data.pendingBuses], ["Routes", data.totalRoutes], ["Local In Service", data.activeLocalServices], ["Scheduled Trips", data.scheduledTrips], ["Completed Trips", data.completedTrips]]} /><div className="grid gap-4 xl:grid-cols-2"><Trend title="Daily trip creation" data={data.dailyTrips} dataKey="count" /><Distribution title="Trip type" values={data.tripTypeBreakdown} /><Distribution title="Trip statuses" values={data.tripStatusBreakdown} /><div className="rounded-2xl border border-slate-200 bg-white p-5"><h2 className="text-xl font-semibold">Management links</h2><div className="mt-4 flex flex-wrap gap-3"><LinkButton to="/admin/buses" label="Manage Buses" /><LinkButton to="/admin/routes" label="Manage Routes" /><LinkButton to="/admin/live-tracking" label="Live Monitoring" /></div></div></div></>;
}

function BookingsDetails({ data }) {
  return <><CardGrid items={[["Total Bookings", data.totalBookings], ["Today", data.bookingsToday], ["Confirmed", data.confirmedBookings], ["Cancelled", data.cancelledBookings]]} /><div className="grid gap-4 xl:grid-cols-2"><Trend title="Daily bookings" data={data.dailyBookings} dataKey="count" /><Distribution title="Booking status" values={data.statusDistribution} /><Distribution title="Trip type" values={data.tripTypeBreakdown} /><Recent title="Recent bookings" rows={data.recentBookings} columns={[["Reference", "bookingReference"], ["Status", "status"], ["Fare", "totalFare"], ["Booked", "bookedAt"]]} /></div></>;
}

function RevenueDetails({ data }) {
  return <><CardGrid money items={[["Ticket Revenue", data.ticketRevenue], ["Wallet Top-ups", data.walletTopUpRevenue], ["Total Verified", data.totalVerifiedRevenue], ["Successful Attempts", data.successfulPayments], ["Pending Attempts", data.pendingPayments], ["Failed Attempts", data.failedPayments]]} /><div className="grid gap-4 xl:grid-cols-2"><Trend title="Daily verified revenue" data={data.dailyRevenue} dataKey="amount" money /><Distribution title="Revenue sources" values={data.sourceBreakdown} money /><Recent title="Recent verified transactions" rows={data.recentTransactions} columns={[["Source", "source"], ["Method", "method"], ["Amount", "amount"], ["Verified", "verifiedAt"]]} /></div></>;
}

function CardGrid({ items, money = false }) {
  return <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{items.map(([label, value]) => <div key={label} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-semibold uppercase tracking-widest text-slate-500">{label}</p><p className="mt-3 text-2xl font-semibold text-slate-900">{money && label.includes("Revenue") || money && label.includes("Top-up") || money && label.includes("Verified") ? formatMoney(value) : Number(value || 0).toLocaleString()}</p></div>)}</section>;
}
...
function Recent({ title, rows, columns }) {
  ...
  return <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-xl font-semibold text-slate-900">{title}</h2>{safeRows.length === 0 ? <Empty /> : <div className="mt-4 overflow-x-auto"><table className="w-full min-w-130 text-left text-sm"><thead><tr className="border-b border-slate-200">{safeColumns.map(([label]) => <th key={label} className="px-3 py-2.5 text-xs font-semibold uppercase text-slate-500">{label}</th>)}</tr></thead><tbody>{safeRows.map((row, index) => <tr key={index} className="border-b border-slate-100">{safeColumns.map(([label, key]) => <td key={label} className="px-3 py-2.5 font-semibold text-slate-700">{formatCell(key, row?.[key])}</td>)}</tr>)}</tbody></table></div>}</div>;
}

function ChartCard({ title, children }) {
  return <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-xl font-semibold text-slate-900">{title}</h2><div className="mt-5 h-64">{children}</div></div>;
}
function LinkButton({ to, label }) { return <Link to={to} className="rounded-xl bg-[#08264a] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#0d3566] focus-visible:outline-2">{label}</Link>; }
...
function Skeleton() { return <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{Array.from({ length: 8 }, (_, index) => <div key={index} className="h-32 animate-pulse rounded-2xl bg-slate-200" />)}</div>; }
