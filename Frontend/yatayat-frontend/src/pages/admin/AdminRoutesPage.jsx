      ...
      <div className="space-y-5">
        <div className="flex flex-col justify-between gap-4 xl:flex-row xl:items-center">
          <div className="min-w-0">
            <h1 className="safe-wrap text-2xl font-semibold text-slate-900">Routes & Local Stops</h1>
          ...
          <div className="grid gap-2 sm:grid-cols-3">
            <button type="button" onClick={openStopCreate} className="tap-target rounded-xl border border-slate-300 px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-white"><Plus size={17} className="inline" /> Add Stop</button>
            <button type="button" onClick={() => openRouteCreate("LOCAL")} className="tap-target rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-emerald-700"><Plus size={17} className="inline" /> Local Route</button>
            <button type="button" onClick={() => openRouteCreate("OUT_OF_VALLEY")} className="tap-target rounded-xl bg-[#08264a] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#0d3566]"><Plus size={17} className="inline" /> Outside Route</button>
          </div>
        </div>
          ...
          <>
            <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
              <div className="flex flex-col justify-between gap-2 lg:flex-row lg:items-end">
                <div>
                  <h2 className="text-xl font-semibold text-slate-900">Route Directory</h2>
                ...
                <label className="relative block w-full lg:max-w-sm">
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-slate-300 bg-slate-50 py-2.5 pl-11 pr-4 text-sm font-bold outline-none focus:border-[#08264a]"
                  />
                </label>
              </div>
        ...
        <Modal title={editingRouteId ? "Edit Route" : routeForm.tripType === "LOCAL" ? "Create Local Route" : "Create Outside Valley Route"} onClose={() => setShowRouteForm(false)} wide>
          <form onSubmit={submitRoute} className="space-y-5">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <label><span className="text-xs font-semibold uppercase tracking-wider text-slate-500">Trip Type</span><select required value={routeForm.tripType} disabled={Boolean(editingRouteId)} onChange={(event) => setRouteForm({ ...routeForm, tripType: event.target.value, stops: event.target.value === "LOCAL" ? defaultStopRows() : [] })} className="mt-2 w-full rounded-xl border border-slate-300 bg-slate-50 px-4 py-2.5 font-bold outline-none"><option value="OUT_OF_VALLEY">Outside Valley</option><option value="LOCAL">Local</option></select></label>
              <label><span className="text-xs font-semibold uppercase tracking-wider text-slate-500">Status</span><select value={routeForm.status} onChange={(event) => setRouteForm({ ...routeForm, status: event.target.value })} className="mt-2 w-full rounded-xl border border-slate-300 bg-slate-50 px-4 py-2.5 font-bold outline-none"><option value="ACTIVE">ACTIVE</option><option value="INACTIVE">INACTIVE</option></select></label>
            ...
            {routeForm.tripType === "LOCAL" && (
              <section className="rounded-2xl border border-emerald-100 bg-emerald-50/60 p-4">
                <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
                  <div><h3 className="font-semibold text-slate-900">Ordered Local Stops</h3><p className="text-sm font-semibold text-slate-600">First stop is boarding origin; last stop is destination. Fare is cumulative from route start.</p></div>
                  <button type="button" onClick={addRouteStop} className="tap-target rounded-xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white">Add Stop</button>
                </div>
                <div className="mt-4 space-y-3">
                  {routeForm.stops.map((stop, index) => (
                    <div key={index} className="grid gap-3 rounded-2xl bg-white p-3 shadow-sm lg:grid-cols-[auto_1fr_1fr_1fr_auto] lg:items-end">
                      ...
                      <label><span className="text-xs font-semibold uppercase text-slate-500">Stop</span><select required value={stop.busStopId} onChange={(event) => updateRouteStop(index, "busStopId", event.target.value)} className="mt-2 w-full rounded-xl border border-slate-300 bg-slate-50 px-3 py-2.5 font-bold"><option value="">Select stop</option>{stops.filter((item) => item.active).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
...
function StopSection({ stops, onEdit, onToggle }) {
  return <section className="space-y-4"><h2 className="text-xl font-semibold text-slate-900">Bus Stops</h2>{stops.length === 0 ? <Empty text="No bus stops created yet." /> : <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{stops.map((stop) => <article key={stop.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h3 className="safe-wrap font-semibold text-slate-900">{stop.name}</h3><p className="safe-wrap mt-1 text-sm font-semibold text-slate-500">{stop.landmark || "No landmark"}</p></div><Status active={stop.active} /></div><p className="mt-4 text-sm font-bold text-slate-600">{coordinate(stop.latitude, stop.longitude)}</p><div className="mt-4 grid gap-2 sm:grid-cols-2"><button type="button" onClick={() => onEdit(stop)} className="tap-target rounded-xl border border-slate-300 px-4 py-2 font-semibold text-slate-700">Edit</button><button type="button" onClick={() => onToggle(stop)} className="tap-target rounded-xl bg-slate-100 px-4 py-2 font-semibold text-slate-700">{stop.active ? "Deactivate" : "Activate"}</button></div></article>)}</div>}</section>;
}
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        {routes.map((route) => (
          <article key={route.id} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex items-start justify-between gap-4"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="rounded-lg bg-blue-50 px-3 py-1 text-xs font-semibold text-[#08264a]">{route.code}</span><Status active={route.status === "ACTIVE"} label={route.status} /><TripTypeBadge tripType={route.tripType} /></div><h3 className="safe-wrap mt-3 text-xl font-semibold text-slate-900">{route.name}</h3></div><div className="flex gap-2"><button type="button" onClick={() => onEdit(route)} disabled={routeStatusBusyId === route.id || deletingRouteId === route.id} className="tap-target rounded-xl border border-slate-200 p-3 text-slate-600 disabled:opacity-50" title="Edit route"><Edit3 size={18} /></button><button type="button" onClick={() => onStatusChange(route)} disabled={routeStatusBusyId === route.id || deletingRouteId === route.id} className="tap-target rounded-xl bg-slate-100 px-3 text-xs font-semibold text-slate-700 disabled:opacity-50">{routeStatusBusyId === route.id ? <Loader2 size={16} className="animate-spin" /> : route.status === "ACTIVE" ? "Deactivate" : "Activate"}</button><button type="button" onClick={() => onDelete(route)} disabled={routeStatusBusyId === route.id || deletingRouteId === route.id} className="tap-target rounded-xl border border-red-200 bg-red-50 p-3 text-red-700 disabled:opacity-50" title="Delete route">{deletingRouteId === route.id ? <Loader2 size={18} className="animate-spin" /> : <Trash2 size={18} />}</button></div></div>
...
function FilterGroup({ label, value, onChange, options }) {
  return <fieldset><legend className="text-xs font-semibold uppercase tracking-wider text-slate-500">{label}</legend><div className="mt-2 flex flex-wrap gap-2">{options.map(([optionValue, optionLabel]) => <button key={optionValue} type="button" aria-pressed={value === optionValue} onClick={() => onChange(optionValue)} className={`rounded-xl px-4 py-2 text-sm font-semibold transition ${value === optionValue ? "bg-[#08264a] text-white shadow-sm" : "border border-slate-200 bg-slate-50 text-slate-600 hover:bg-slate-100"}`}>{optionLabel}</button>)}</div></fieldset>;
}
function Modal({ title, onClose, children, wide = false }) {
  return <div className="responsive-modal-backdrop fixed inset-0 z-50 flex justify-center bg-black/50 sm:items-center"><div className={`responsive-modal-panel w-full ${wide ? "max-w-5xl" : "max-w-2xl"} rounded-2xl bg-white p-5 shadow-2xl sm:p-6`}><div className="mb-6 flex items-start justify-between gap-4"><div className="min-w-0"><h2 className="safe-wrap text-xl font-semibold sm:text-2xl">{title}</h2><p className="mt-1 text-sm text-slate-500">Admin-only local travel foundation controls.</p></div><button type="button" aria-label="Close form" onClick={onClose} className="tap-target rounded-xl p-2 hover:bg-slate-100"><X /></button></div>{children}</div></div>;
}

function Field({ label, value, onChange, type = "text", step, required = true }) {
  return <label><span className="text-xs font-semibold uppercase tracking-wider text-slate-500">{label}</span><input required={required} type={type} min={type === "number" ? "0" : undefined} step={step} value={value} onChange={(event) => onChange(event.target.value)} className="mt-2 w-full rounded-xl border border-slate-300 bg-slate-50 px-4 py-2.5 font-bold outline-none focus:border-[#08264a]" /></label>;
}

function SaveButton({ saving, label }) {
  return <button type="submit" disabled={saving} className="tap-target flex w-full items-center justify-center gap-2 rounded-xl bg-[#08264a] py-2.5 font-semibold text-white disabled:opacity-60">{saving && <Loader2 size={18} className="animate-spin" />}{saving ? "Saving..." : label}</button>;
}
...
function Info({ label, value }) {
  return <div className="rounded-xl border border-slate-100 p-3"><p className="text-xs font-semibold uppercase text-slate-400">{label}</p><p className="safe-wrap mt-1 font-semibold text-slate-800">{value}</p></div>;
}

function Status({ active, label }) {
  return <span className={`rounded-full px-3 py-1 text-xs font-semibold ${active ? "bg-emerald-100 text-emerald-700" : "bg-slate-200 text-slate-600"}`}>{label || (active ? "ACTIVE" : "INACTIVE")}</span>;
}

function TripTypeBadge({ tripType }) {
  const local = tripType === "LOCAL";
  return <span className={`rounded-full px-3 py-1 text-xs font-semibold ${local ? "bg-amber-100 text-amber-700" : "bg-violet-100 text-violet-700"}`}>{local ? "Local" : "Outside Valley"}</span>;
}

function Empty({ text }) {
  return <div className="rounded-2xl border border-slate-200 bg-white p-6 text-center"><RouteIcon className="mx-auto text-slate-400" size={44} /><p className="mt-4 font-semibold text-slate-700">{text}</p></div>;
}

function LoadingCard() {
  return <div className="flex min-h-40 items-center justify-center rounded-2xl border border-slate-200 bg-white"><Loader2 className="animate-spin" size={32} /></div>;
}
