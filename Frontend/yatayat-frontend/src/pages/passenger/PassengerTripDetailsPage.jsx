...
export default function PassengerTripDetailsPage() {
  ...
  return <PassengerLayout activePage="Find Trips"><div className="mx-auto max-w-5xl space-y-6">
    ...
    {loading ? <div className="flex min-h-72 items-center justify-center"><Loader2 className="animate-spin" size={40} /></div> : trip && <>
      ...
      {trip.tripType === "LOCAL" ? <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5"><p className="font-bold text-amber-900">Local fare ticket support is coming in a later phase. Seat reservations are not available for local trips.</p><button disabled className="mt-4 cursor-not-allowed rounded-xl bg-amber-200 px-6 py-3 font-black text-amber-800">Local Fare Ticket - Coming Next</button></div> : <div className="rounded-2xl border border-blue-200 bg-blue-50 p-5"><p className="font-bold text-blue-900">Continue to enter passenger details. Payment and ticket generation are not part of this booking step.</p><button onClick={() => navigate(`/passenger/trips/${tripId}/book`)} className="mt-4 rounded-xl bg-[#08264a] px-6 py-3 font-black text-white">Continue to Booking</button></div>}
    </>}
  </div></PassengerLayout>;
}
