...
export default function PassengerTripSearchPage() {
  ...
  return <PassengerLayout activePage="Find Trips"><div className="space-y-6">
    <header><h1 className="text-3xl font-black text-slate-900">Find Scheduled Trips</h1><p className="mt-1 text-sm text-slate-500">Search real services created by approved transport operators.</p></header>
    ...
    {loading ? <div className="flex min-h-64 items-center justify-center"><Loader2 className="animate-spin" size={40} /></div> : searched && trips.length === 0 && !error ? <div className="rounded-3xl border border-slate-200 bg-white py-16 text-center"><Bus className="mx-auto text-slate-300" size={48} /><h2 className="mt-4 text-xl font-black">No scheduled trips found</h2><p className="mt-2 text-sm text-slate-500">Try another date or route direction.</p></div> : <div className="space-y-4">{trips.map((trip) => <TripCard key={trip.tripId} trip={trip} onView={() => navigate(`/passenger/trips/${trip.tripId}`)} />)}</div>}
  </div></PassengerLayout>;
}
