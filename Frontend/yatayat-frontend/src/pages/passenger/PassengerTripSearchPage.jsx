...
export default function PassengerTripSearchPage() {
  ...
  return <PassengerLayout activePage="Book Out-of-Valley"><div className="space-y-6">
    <header><p className="text-xs font-black uppercase tracking-widest text-violet-700">{t("passenger.booking.outOfValleyEyebrow")}</p><h1 className="mt-1 text-3xl font-black text-slate-900">{t("passenger.booking.searchTitle")}</h1><p className="mt-1 text-sm text-slate-500">{t("passenger.booking.searchSubtitle")}</p></header>
    <form onSubmit={searchTrips} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><div className="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_1fr_0.8fr_auto] lg:items-end"><Field icon={<MapPin size={18} />} label={t("passenger.booking.fromOrigin")} value={form.origin} onChange={(value) => setForm((current) => ({ ...current, origin: value }))} placeholder="Kathmandu" required /><Field icon={<MapPin size={18} />} label={t("passenger.booking.toDestination")} value={form.destination} onChange={(value) => setForm((current) => ({ ...current, destination: value }))} placeholder="Pokhara" required /><Field icon={<CalendarDays size={18} />} label={t("passenger.booking.departureDateOptional")} type="date" value={form.date} onChange={(value) => setForm((current) => ({ ...current, date: value }))} /><div className="flex gap-2"><button disabled={loading} className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-[#08264a] px-5 py-3 font-black text-white disabled:opacity-60"><Search size={18} /> {t("passenger.booking.search")}</button><button type="button" onClick={clear} title={t("passenger.booking.clearSearch")} aria-label={t("passenger.booking.clearSearch")} className="rounded-xl border border-slate-300 p-3 text-slate-600"><RotateCcw size={18} /></button></div></div></form>
